...
public abstract non-sealed class AreaBasePlacement implements AreaPlacement {
    ...
    protected final Supplier<List<Vec>> blockPositions;
    ...
    AreaBasePlacement(@NotNull Supplier<List<Vec>> blockPositions, @NotNull Supplier<List<Vec>> tntPositions, @NotNull BlockPlaceFunction blockPlaceFunction) {
    ...
    protected void sendExpCount(int currentSize) {
        for (Player onlinePlayer : MinecraftServer.getConnectionManager().getOnlinePlayers()) {
            float progress = (float) currentSize / this.blockPositions.get().size();
